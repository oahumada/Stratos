...
export async function mockLLMRoutes(page: Page, options?: { fixture?: string; generationId?: number }) {
  const generationId = options?.generationId ?? 12345;
  const fixturePath = options?.fixture || path.resolve(__dirname, '..', 'fixtures', 'llm', 'mock_generation_response.json');
  let fixture: any = {};
  try {
  ...
  await page.route('**/api/strategic-planning/scenarios/generate', async (route) => {
    const body = { success: true, data: { id: generationId, status: 'queued', url: `/api/strategic-planning/scenarios/generate/${generationId}` } };
    await route.fulfill({ status: 202, contentType: 'application/json', body: JSON.stringify(body) });
  });

  await page.route(`**/api/strategic-planning/scenarios/generate/${generationId}`, async (route) => {
    const body = {
      success: true,
      data: {
        id: generationId,
