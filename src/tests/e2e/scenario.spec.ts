...
test('scenario map - click capability expands children and centers selected node', async ({
    page,
    baseURL,
}, testInfo) => {
    ...
    try {
        ...
        if (/login|sanctum|unauthorized/i.test(currentUrl)) {
            ...
            await testInfo.attach('redirect-html', {
                body: html,
            ...
            throw new Error(
                `Navigated to login/unauthorized page: ${currentUrl}`,
            );
        }
        ...
        const capLocator = page
            .locator('svg .node-group:not(.scenario-node)')
            .first();
