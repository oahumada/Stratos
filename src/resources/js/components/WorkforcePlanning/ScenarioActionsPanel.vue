...
const decisionTransitions = computed(() => {
  ...
  const buttons: Array<{ label: string; color: string; icon: string; toStatus: string; disabled: boolean }> = []
...
const executionActions = computed(() => {
  ...
  const buttons: Array<{ label: string; color: string; icon: string; action: string; disabled: boolean; tooltip?: string }> = []
...
const canCreateVersion = computed(() => {
  return props.scenario.decision_status === 'approved' && props.scenario.is_current_version
})
...
const canSyncFromParent = computed(() => {
  return props.scenario.parent_id !== null && props.scenario.decision_status !== 'approved'
})
...
const confirmTransition = async () => {
  loading.value = true
  try {
    await api.post(`/api/v1/workforce-planning/scenarios/${props.scenario.id}/decision-status`, {
...
const executeAction = async (action: string) => {
  loading.value = true
  try {
    await api.post(`/api/v1/workforce-planning/scenarios/${props.scenario.id}/execution/${action}`, {
...
const syncFromParent = async () => {
  loading.value = true
  try {
    const res = await api.post(`/api/v1/workforce-planning/scenarios/${props.scenario.id}/sync-parent`)
...
const createNewVersion = async (payload: any) => {
  loading.value = true
  try {
    const res = await api.post(`/api/v1/workforce-planning/scenarios/${props.scenario.id}/versions`, payload)
    showSuccess(res.message || 'Nueva versión creada')
    showVersionDialog.value = false
    emit('refresh')
    // Redirigir a la nueva versión
    if (res.data?.id) {
      window.location.href = `/workforce-planning/scenarios/${res.data.id}`
    }
  } catch (error: any) {
...
const decisionStatusBadge = computed(() => {
  ...
  const map: Record<string, { color: string; text: string; icon: string }> = {
    draft: { color: 'grey', text: 'Borrador', icon: 'mdi-file-document-edit' },
    pending_approval: { color: 'warning', text: 'Pendiente Aprobación', icon: 'mdi-clock-alert' },
    approved: { color: 'success', text: 'Aprobado', icon: 'mdi-check-circle' },
    rejected: { color: 'error', text: 'Rechazado', icon: 'mdi-close-circle' }
  }
...
const executionStatusBadge = computed(() => {
  ...
  const map: Record<string, { color: string; text: string; icon: string }> = {
    planned: { color: 'grey-lighten-1', text: 'Planificado', icon: 'mdi-calendar-clock' },
    in_progress: { color: 'primary', text: 'En Ejecución', icon: 'mdi-play-circle' },
    paused: { color: 'warning', text: 'Pausado', icon: 'mdi-pause-circle' },
    completed: { color: 'success', text: 'Completado', icon: 'mdi-check-circle-outline' }
  }
...
<template>
  <div class="scenario-actions-panel">
    ...
    <v-row v-if="decisionTransitions.length > 0">
      <v-col cols="12">
        <div class="text-subtitle-2 mb-2 text-medium-emphasis">Transiciones de Estado</div>
        <v-btn
    ...
    <v-row v-if="executionActions.length > 0">
      <v-col cols="12">
        <div class="text-subtitle-2 mb-2 text-medium-emphasis">Control de Ejecución</div>
        <v-btn
    ...
    <v-row>
      <v-col cols="12">
        ...
        <div class="text-subtitle-2 mb-2 text-medium-emphasis">Acciones Adicionales</div>
    ...
    <v-dialog v-model="showTransitionDialog" max-width="500">
      <v-card>
        ...
        <v-card-text>
          <p>¿Confirmas transicionar el estado a <strong>{{ targetStatus }}</strong>?</p>
          <v-textarea
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showTransitionDialog = false">Cancelar</v-btn>
          <v-btn color="primary" :loading="loading" @click="confirmTransition">Confirmar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showExecutionDialog" max-width="500">
      <v-card>
        <v-card-title>Confirmar {{ targetStatus === 'start' ? 'Inicio' : targetStatus === 'pause' ? 'Pausa' : 'Finalización' }}</v-card-title>
        <v-card-text>
          <v-textarea
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showExecutionDialog = false">Cancelar</v-btn>
          <v-btn color="primary" :loading="loading" @click="executeAction(targetStatus)">Confirmar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-card>
        ...
        <v-card-text>
          <p class="text-body-2 text-medium-emphasis mb-4">
            Se creará una copia editable del escenario actual. El escenario aprobado quedará inmutable.
          </p>
          <v-text-field
            v-model="versionName"
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showVersionDialog = false">Cancelar</v-btn>
          <v-btn
            color="purple"
            :loading="loading"
            :disabled="!versionName"
            @click="createNewVersion({ name: versionName, description: versionDescription, notes: versionNotes, copy_skills: copySkills, copy_strategies: copyStrategies })"
          >
            Crear Versión
          </v-btn>
        </v-card-actions>
