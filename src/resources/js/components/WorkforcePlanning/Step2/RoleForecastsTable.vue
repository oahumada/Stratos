<template>
  <div class="role-forecasts-container">
    <div class="mb-6">
      <h3 class="text-h5 font-semibold mb-4">Pronósticos de Roles</h3>
      <p class="text-sm text-gray-600 mb-4">
        Visualiza el cambio proyectado en cantidad de FTE y evolución de roles
      </p>
    </div>
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full border-collapse">
        <thead class="bg-gray-100 sticky top-0 z-10">
          <tr>
            <th class="border px-4 py-3 text-left font-semibold">Rol</th>
            <th class="border px-4 py-3 text-center font-semibold">FTE Actual</th>
            <th class="border px-4 py-3 text-center font-semibold">FTE Futuro</th>
            <th class="border px-4 py-3 text-center font-semibold">Δ FTE</th>
            <th class="border px-4 py-3 text-left font-semibold">Tipo Evolución</th>
            <th class="border px-4 py-3 text-left font-semibold">Impacto</th>
            <th class="border px-4 py-3 text-center font-semibold">Acciones</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="forecast in forecasts" :key="forecast.id" class="hover:bg-gray-50">
            <td class="border px-4 py-3 font-medium">{{ forecast.role_name }}</td>
            <td class="border px-4 py-3 text-center">{{ forecast.fte_current }}</td>
            ...
            <td class="border px-4 py-3 text-center">
              ...
              >
                {{ forecast.fte_delta > 0 ? '+' : '' }}{{ forecast.fte_delta }}
              </span>
            </td>
            <td class="border px-4 py-3">
              <v-chip
                :color="getEvolutionColor(forecast.evolution_type)"
                size="small"
                variant="tonal"
              >
    ...
    <v-dialog v-model="showEditDialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="saveForecast" :loading="saving">
            Guardar
          </v-btn>
        </v-card-actions>
      </v-card>
...
const fteDelta = computed(() => {
  if (!editData.value) return 0;
  return editData.value.fte_future - editData.value.fte_current;
});
