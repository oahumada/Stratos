<template>
  <div class="succession-plan-container">
    <div class="mb-6">
      <h3 class="text-h5 font-semibold mb-4">Plan de Sucesión</h3>
      <p class="text-sm text-gray-600 mb-4">
        Identificación de sucesores y timeline de transición para posiciones críticas
      </p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-6">
      <div v-if="plans.length === 0" class="col-span-full text-center py-8 text-gray-500">
      ...
      >
        ...
        <div class="bg-gradient-to-r from-blue-50 to-blue-100 p-4 border-b">
          <div class="flex justify-between items-start mb-2">
            <div>
              <h4 class="text-lg font-semibold text-gray-800">{{ plan.position_name }}</h4>
              <p class="text-sm text-gray-600">{{ plan.department }}</p>
            </div>
            <v-chip :color="getCriticalityColor(plan.criticality)" size="small" variant="tonal">
        ...
        <div class="p-4 bg-gray-50 border-b">
          <p class="text-xs font-semibold text-gray-600 mb-2">TITULAR ACTUAL:</p>
          <div class="flex items-center justify-between">
            <div>
              <p class="font-semibold text-gray-800">{{ plan.current_holder_name }}</p>
              <p class="text-sm text-gray-600">Años en posición: {{ plan.years_in_position }}</p>
            </div>
            <div class="text-right">
              <p class="text-sm font-semibold text-gray-700">Edad: {{ plan.current_holder_age }}</p>
              <p class="text-xs text-gray-500">Retiro estimado: {{ plan.estimated_retirement }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 space-y-3">
          <p class="text-xs font-semibold text-gray-600 uppercase">Sucesores Identificados:</p>

          <div v-if="plan.successors && plan.successors.length > 0" class="space-y-3">
            <div
            ...
            >
              <div class="flex items-start justify-between mb-2">
                <div>
                  <p class="font-semibold text-gray-800">{{ idx + 1 }}. {{ successor.name }}</p>
                  <p class="text-sm text-gray-600">{{ successor.current_role }}</p>
                </div>
                <v-chip :color="getReadinessColor(successor.readiness_level)" size="small">
                  {{ formatReadiness(successor.readiness_level) }}
                </v-chip>
              </div>
              ...
              <v-progress-linear
                ...
                :color="getReadinessColor(successor.readiness_level)"
                height="6"
                class="mb-2"
              />
              ...
              <div class="flex gap-2 flex-wrap mb-2">
                <v-chip
                  v-for="gap in successor.skill_gaps.slice(0, 2)"
                  :key="gap.id"
                  size="small"
                  variant="tonal"
                  color="warning"
                ...
                <v-chip
                  ...
                  color="gray"
                >
                  +{{ successor.skill_gaps.length - 2 }} skills
                </v-chip>
              </div>

              <!-- Timeline -->
              <div class="text-xs text-gray-600">
                <p><strong>Timeline de Preparación:</strong> {{ successor.timeline_months }} meses</p>
                <p v-if="successor.development_plan">
                  <strong>Plan de Desarrollo:</strong> {{ successor.development_plan }}
                </p>
              </div>
            </div>
          </div>

          <div v-else class="p-3 bg-red-50 rounded border border-red-200">
        ...
        <div class="p-4 bg-gray-50 border-t flex gap-2">
          <v-btn size="small" variant="tonal" color="primary" @click="editPlan(plan)">
            Editar Plan
          </v-btn>
          <v-btn size="small" variant="tonal" @click="viewTimeline(plan)">
            Ver Timeline
          </v-btn>
          <v-spacer />
          <v-btn size="small" variant="text" color="error" @click="deletePlan(plan.id)">
            Eliminar
          </v-btn>
        </div>
      </div>
    ...
    <v-dialog v-model="showEditDialog" max-width="600px">
      <v-card v-if="editingPlan">
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="savePlan" :loading="saving">Guardar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
interface Successor {
  ...
  name: string;
  current_role: string;
  readiness_level: 'ready_now' | 'ready_12_months' | 'ready_24_months' | 'not_ready';
  ...
  try {
    ...
    const response = await fetch(
      `/api/scenarios/${props.scenarioId}/step2/succession-plans/${editingPlan.value.id}`,
      {
        ...
        body: JSON.stringify({ notes: editingPlan.value.notes, criticality: editingPlan.value.criticality }),
      }
    );
