...
const loadParentCandidates = async () => {
  ...
  try {
    ...
    const res = await api.get(`/api/v1/workforce-planning/scenarios?${params.toString()}`)
    ...
    >
      <template #item="{ props: itemProps, item }">
        <v-list-item v-bind="itemProps">
          ...
          <v-list-item-subtitle v-if="item.raw.description" class="text-wrap">
          ...
          <template #append>
            <div class="d-flex flex-column align-end">
              <v-chip
                :color="getStatusBadge(item.raw.decision_status).color"
                size="x-small"
                variant="flat"
                class="mb-1"
              >
                {{ getStatusBadge(item.raw.decision_status).text }}
              </v-chip>
              <v-chip
                v-if="item.raw.children_count && item.raw.children_count > 0"
                size="x-small"
                variant="outlined"
