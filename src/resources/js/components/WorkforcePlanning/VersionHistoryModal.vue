    ...
    <v-card>
      ...
      <v-card-text style="max-height: 600px;">
        <v-alert
          ...
          class="mb-4"
        >
          No hay versiones anteriores. Esta es la primera versión del escenario.
        </v-alert>
        ...
        >
          <div class="d-flex align-center">
            <span>{{ selectedVersions.length }} versiones seleccionadas para comparar</span>
            <v-spacer />
            <v-btn
              v-if="canCompare"
              color="primary"
        ...
        <v-timeline side="end" density="compact" line-inset="8">
          <v-timeline-item
            ...
            :key="version.id"
            :dot-color="version.is_current_version ? 'primary' : 'grey'"
            size="small"
          >
            <template #icon>
              <v-icon
                :icon="version.is_current_version ? 'mdi-star' : 'mdi-circle'"
                size="small"
              />
            </template>
            ...
            <v-card
              ...
              :class="{ 'border-primary': version.is_current_version }"
              class="mb-3"
            >
              <v-card-title class="d-flex align-center">
                <div class="d-flex flex-column flex-grow-1">
                  <div class="d-flex align-center">
                    <v-chip
                      :color="version.is_current_version ? 'primary' : 'grey'"
                      size="small"
                      variant="flat"
                      class="mr-2"
                    >
                    ...
                    <span class="text-h6">{{ version.name }}</span>
                  </div>
                  <div class="text-caption text-medium-emphasis mt-1">
                    Creado: {{ formatDate(version.created_at) }}
                    <span v-if="version.owner"> · {{ version.owner.name }}</span>
                  </div>
                </div>

                <v-checkbox
                  :model-value="selectedVersions.includes(version.id)"
                  :disabled="!canCompare && !selectedVersions.includes(version.id)"
                  density="compact"
                  hide-details
                  @update:model-value="toggleCompare(version.id)"
                />
              </v-card-title>

              <v-card-text v-if="version.description">
                <p class="text-body-2 mb-2">{{ version.description }}</p>
              </v-card-text>

              <v-card-text>
                <div class="d-flex gap-2 flex-wrap">
                  <v-chip
                    :color="getVersionBadgeColor(version.decision_status)"
                    size="small"
                    variant="tonal"
                  >
                  ...
                  <v-chip
                    :color="getExecutionBadgeColor(version.execution_status)"
                    size="small"
                    variant="outlined"
                  >
