...
export const useWorkforcePlanningStore = defineStore('workforcePlanning', {
    ...
    getters: {
        ...
        getFilteredForecasts: (state) => (scenarioId: number) => {
            ...
            if (state.filters.forecastArea) {
                filtered = filtered.filter(f => f.area === state.filters.forecastArea)
            }

            if (state.filters.forecastCriticality) {
                filtered = filtered.filter(f => f.criticality_level === state.filters.forecastCriticality)
            }
        ...
        getFilteredMatches: (state) => (scenarioId: number) => {
            ...
            if (state.filters.matchReadiness) {
                filtered = filtered.filter(m => m.readiness_level === state.filters.matchReadiness)
            }

            if (state.filters.matchScoreRange) {
                filtered = filtered.filter(m => {
                    if (state.filters.matchScoreRange === 'high') return m.match_score >= 80
                    if (state.filters.matchScoreRange === 'medium') return m.match_score >= 60 && m.match_score < 80
                    if (state.filters.matchScoreRange === 'low') return m.match_score < 60
                    return true
                })
            }
        ...
        getFilteredSkillGaps: (state) => (scenarioId: number) => {
            ...
            if (state.filters.gapPriority) {
                filtered = filtered.filter(g => g.priority === state.filters.gapPriority)
            }

            if (state.filters.gapDepartment) {
                filtered = filtered.filter(g => g.department === state.filters.gapDepartment)
            }
        ...
        // Loading states
        isLoading: (state) => Object.values(state.loading).some(v => v === true),
        getLoadingState: (state) => (key: string) => state.loading[key as keyof typeof state.loading],
    ...
    actions: {
        ...
        async fetchScenarios() {
            ...
            try {
                const response = await axios.get('/api/strategic-planning/scenarios')
        ...
        async fetchScenario(id: number) {
            try {
                const response = await axios.get(`/api/strategic-planning/scenarios/${id}`)
