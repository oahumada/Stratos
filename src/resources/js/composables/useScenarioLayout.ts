...
import { chooseMatrixVariant, computeCompetencyMatrixPositions, computeSidesPositions, decideCompetencyLayout } from './useCompetencyLayout';
...
export function useScenarioLayout() {
  ...
  function clampY(y: number) {
    ...
    const maxY = Math.max(minViewportHeight, (window as any).__scenarioHeight ?? 600) - bottomPadding;
  ...
  function wrapLabel(s: any, max = 14) {
    ...
    const cutLine = (text: string, limit: number) => {
      ...
      if (lastSpace > 0) {
        return { line: text.slice(0, lastSpace), rest: text.slice(lastSpace + 1).trim() };
      }
    ...
    if (secondCut.rest && second.length >= max) {
      second = second.slice(0, Math.max(0, max - 1)).replace(/\s+$/, '') + '…';
  ...
  function computeInitialPosition(idx: number, total: number, width: number, height: number) {
  ...
  // ==================== CENTER ON NODE ====================
  function centerOnNode(node: NodeItem, prev: NodeItem | undefined, width: number, height: number) {
    if (!node) return;
  ...
  function expandCompetencies(
    ...
    opts: { limit?: number; rows?: number; cols?: number; layout?: 'auto' | 'radial' | 'matrix' | 'sides' } = {},
    width?: number,
    height?: number
  ) {
    const comps = (node as any).competencies ?? [];
    if (!Array.isArray(comps) || comps.length === 0) return { childNodes: [], childEdges: [] };
    ...
    const defaultParentOffset = LAYOUT_CONFIG.competency.spacing.parentOffset;
    ...
    const layout = decideCompetencyLayout(opts.layout, false, toShow.length, LAYOUT_CONFIG.competency.defaultLayout);
    ...
    if (layout === 'radial') {
      ...
      try {
        positions = computeSidesPositions(toShow.length, cx, parentY, LAYOUT_CONFIG.competency.sides, null);
      } catch (err) {
        positions = [];
      }
    } else {
    ...
    toShow.forEach((c: any, i: number) => {
      ...
      const delay = Math.max(
        0,
        Math.floor(i / cols) * (LAYOUT_CONFIG.animations.competencyStaggerRow ?? 30) +
          (i % cols) * (LAYOUT_CONFIG.animations.competencyStaggerCol ?? 12) +
          Math.round((Math.random() - 0.5) * (LAYOUT_CONFIG.animations.competencyStaggerRandom ?? 30))
      );
  ...
  function expandSkills(
    ...
    if (!Array.isArray(skills) || skills.length === 0) return { grandChildNodes: [], grandChildEdges: [] };
    ...
    const SKILL_DROP_EXTRA = 18;
    const topY = Math.round(parentY + SKILL_PARENT_OFFSET + SKILL_DROP_EXTRA);

    const layoutOpt = opts?.layout ?? undefined;
    const layout = decideCompetencyLayout(layoutOpt as any, false, toShow.length, LAYOUT_CONFIG.skill.defaultLayout as any);
    ...
    if (layout === 'radial') {
      ...
      try {
        positions = computeSidesPositions(toShow.length, cx, parentY, LAYOUT_CONFIG.skill.sides, null);
      } catch (err) {
        positions = [];
      }
    } else {
  ...
  function runForceLayout(
    ...
    try {
      ...
      const pos = new Map(simNodes.map((n: any) => [n.id, { x: Math.round(n.x), y: Math.round(n.y) }]));
