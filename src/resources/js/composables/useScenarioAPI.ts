...
export function useScenarioAPI(options?: ScenarioAPIOptions) {
  ...
  // ==================== SAVE CAPABILITY ====================
  async function saveCapability(id: number, payload: any, scenarioId?: number) {
    await ensureCsrf();
    ...
    try {
      ...
      if (scenarioId) {
        ...
        try {
          ...
          try {
            ...
            console.error('[saveCapability] error POST fallback', err2);
      ...
      console.error('[saveCapability] error PATCH /api/capabilities/', _err?.response?.data ?? _err);
  ...
  async function deleteCapability(id: number, scenarioId?: number) {
    try {
      ...
      if ((pivotErrStatus === 404 || pivotErrStatus === null) && (capErrStatus === 404 || capErrStatus === null)) {
        showError('Eliminado localmente. El backend no expone endpoints DELETE.');
      } else {
  ...
  // ==================== DELETE COMPETENCY ====================
  async function deleteCompetency(compId: number, capabilityId: number, scenarioId?: number) {
    try {
      ...
      if (_e?.response?.status === 404) {
        ...
        console.debug('[deleteCompetency] DELETE 404, removing locally');
        return true;
      }
  ...
  async function fetchSkillsForCompetency(compId: number): Promise<any[]> {
    ...
    try {
          ...
          for (const cap of items) {
            ...
            for (const comp of cap.competencies) {
              if (Number(comp.id) === Number(compId)) {
                return Array.isArray(comp.skills) ? comp.skills : [];
              }
            }
          }
        } catch (err) {
          console.debug('[fetchSkillsForCompetency] capability-tree failed', err);
        }
      }
      ...
      try {
        ...
        console.debug('[fetchSkillsForCompetency] /competencies/:id/skills failed', err);
      }
      ...
      try {
        ...
        console.debug('[fetchSkillsForCompetency] /competencies/:id failed', err);
      }
  ...
  // ==================== DELETE SKILL ====================
  async function deleteSkill(skillId: number, compId?: number, pivotId?: number) {
    try {
      ...
      if (compId && skillId) {
        try {
          ...
          console.debug('[deleteSkill] DELETE via competency failed', e);
        }
      }
  ...
  async function createCompetency(
    ...
    try {
      console.debug('[createCompetency] POST', { scenarioId, capabilityId, competencyPayload });
