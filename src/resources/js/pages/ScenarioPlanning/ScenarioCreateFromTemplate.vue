...
const loadTemplates = async () => {
    ...
    try {
        const res: any = await api.get(
            '/api/strategic-planning/scenario-templates',
        );
        templates.value = Array.isArray(res?.data)
            ? res.data
            : Array.isArray(res)
              ? res
              : [];
...
const applyTemplateDefaults = () => {
    ...
    customizations.value.scenario_type = allowed.includes(tplType)
        ? tplType
        : 'growth';
};
...
const createScenario = async () => {
    ...
    try {
        ...
        console.debug('instantiate-from-template payload', {
            customizations: customizations.value,
        });
        const res: any = await api.post(
            `/api/strategic-planning/scenarios/${selectedTemplate.value.id}/instantiate-from-template`,
            {
                customizations: customizations.value,
            },
        );
        ...
        try {
            const plan =
                res?.workforce_plan ?? res?.data?.workforce_plan ?? null;
            if (plan) {
                ...
                localStorage.setItem(
                    ...
                    JSON.stringify({
                        name: plan.name,
                        scope_type:
                            plan.scope_type === 'organization_wide'
                                ? 'organization'
                        ...
                        time_horizon_weeks:
                            (plan.planning_horizon_months || 0) * 4,
                    }),
                );
            } else {
                localStorage.setItem(
                    ...
                    JSON.stringify({
                        ...
                        time_horizon_weeks:
                            customizations.value.time_horizon_weeks,
                    }),
                );
            }
        } catch (e) {
        ...
        console.error(
            'instantiate-from-template error:',
        ...
        <v-card-title class="d-flex align-center justify-space-between">
            <div>
                ...
                <div class="text-caption text-medium-emphasis">
                    Selecciona una plantilla y ajusta los parámetros clave
                </div>
            </div>
            ...
            <v-col cols="12" md="4">
                ...
                >
                    ...
                    >
                        <template #prepend>
                            <v-icon
                                :icon="tpl.icon || 'mdi-shape-outline'"
                                color="primary"
                                class="mr-2"
                            />
                        </template>
                ...
                <div v-else>
                    ...
                    <v-row>
                        <v-col cols="12" md="4">
                            <v-text-field
                                v-model.number="
                                    customizations.time_horizon_weeks
                                "
                                type="number"
                    ...
                    <v-chip-group
                        v-if="selectedTemplate?.config?.predefined_skills"
                        column
                    >
                        <v-chip
                            v-for="skill in selectedTemplate.config
                                .predefined_skills"
                            ...
                            class="ma-1"
                        >
                            {{ skill.skill || 'Skill #' + skill.skill_id }} -
