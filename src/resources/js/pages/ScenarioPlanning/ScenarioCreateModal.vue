...
const decisionStatus = ref<'draft' | 'pending_approval' | 'approved' | 'rejected'>('draft')
const executionStatus = ref<'planned' | 'in_progress' | 'paused' | 'completed'>('planned')
...
const createScenario = async () => {
  ...
  try {
    ...
    const res: any = await api.post('/api/strategic-planning/scenarios', payload)
  ...
  <v-card>
    ...
    <v-card-text>
      <v-row>
        ...
        <v-col cols="12" md="6">
          <v-select v-model="decisionStatus" :items="decisionOptions" item-title="title" item-value="value" label="Estado (Decisión)" />
        </v-col>

        <v-col cols="12" md="6">
          <v-select v-model="executionStatus" :items="executionOptions" item-title="title" item-value="value" label="Estado (Ejecución)" />
        </v-col>

        <v-col cols="12" md="6">
          <v-text-field v-model.number="versionNumber" label="Versión" type="number" />
        </v-col>

        <v-col cols="12" md="6">
          <v-text-field v-model.number="timeHorizonWeeks" label="Horizonte (semanas)" type="number" />
        </v-col>
      </v-row>
    </v-card-text>
    <v-card-actions>
      ...
      <v-btn color="primary" :loading="saving" @click="createScenario">Crear</v-btn>
    </v-card-actions>
  </v-card>
</template>
