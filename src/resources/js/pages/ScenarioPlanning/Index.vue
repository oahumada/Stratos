<template>
  <div class="prototype-map-root">
    ...
    <div v-else>
      <svg :width="width" :height="height" :viewBox="`0 0 ${width} ${height}`" class="map-canvas" style="touch-action: none;">
        <defs>
          <linearGradient id="bgGrad" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stop-color="#071029" stop-opacity="1"/>
            <stop offset="100%" stop-color="#0b1f33" stop-opacity="1"/>
          </linearGradient>

          <radialGradient id="nodeGrad" cx="45%" cy="30%" r="70%">
            <stop offset="0%" stop-color="#ffffff" stop-opacity="0.18" />
            <stop offset="40%" stop-color="#6fc3ff" stop-opacity="0.95" />
        ...
        <!-- edges -->
        <g class="edges">
          <line v-for="(e, idx) in edges" :key="`edge-${idx}`"
...
interface NodeItem { id: number; name: string; x: number; y: number; is_critical?: boolean }
...
function buildNodesFromItems(items: any[]) {
  ...
  const mapped = items.map((it: any, idx: number) => {
    ...
    const fallbackX = Math.round(centerX + radius * Math.cos(idx * angleStep))
    const fallbackY = Math.round(centerY + radius * Math.sin(idx * angleStep))
    return { id: it.id, name: it.name, x: x ?? fallbackX, y: y ?? fallbackY, _hasCoords: hasPos, is_critical: it.is_critical ?? it.isCritical ?? false }
  })
  nodes.value = mapped.map((m: any) => ({ id: m.id, name: m.name, x: m.x, y: m.y, is_critical: !!m.is_critical }))
...
function runForceLayout() {
  try {
    ...
    const simNodes = nodes.value.map((n) => ({ id: n.id, x: n.x || 0, y: n.y || 0 }))
    const simLinks = edges.value.map((l) => ({ source: l.source, target: l.target }))
    const simulation = d3
      .forceSimulation(simNodes as any)
      .force('link', (d3 as any).forceLink(simLinks).id((d: any) => d.id).distance(120).strength(0.5))
...
.node-group:hover { transform: scale(1.04); }

.node-circle {
  transition: r 0.12s ease, filter 0.12s ease, transform 0.12s ease;
}
...
/* critical pulse */
@keyframes pulse {
  0% { transform: scale(1); filter: drop-shadow(0 0 0 rgba(255,0,0,0.0)); }
  50% { transform: scale(1.08); filter: drop-shadow(0 0 12px rgba(255,60,60,0.65)); }
  100% { transform: scale(1); filter: drop-shadow(0 0 0 rgba(255,0,0,0.0)); }
}
