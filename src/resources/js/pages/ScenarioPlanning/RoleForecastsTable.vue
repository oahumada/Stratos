<template>
  <div class="role-forecasts-table">
    <v-card class="mb-4">
      ...
      <v-card-subtitle>
        Projected workforce needs by role over {{ forecastMonths }} months
      </v-card-subtitle>
      ...
      >
        ...
        <template v-slot:item.critical_skills="{ item }">
          <div class="d-flex flex-wrap gap-2">
            <v-chip
              v-for="skill in (item.critical_skills || []).slice(0, 2)"
              :key="skill"
              size="small"
              variant="outlined"
              color="error"
        ...
        <template v-slot:item.emerging_skills="{ item }">
          <div class="d-flex flex-wrap gap-2">
            <v-chip
              v-for="skill in (item.emerging_skills || []).slice(0, 2)"
              :key="skill"
              size="small"
              variant="outlined"
              color="warning"
          ...
          <v-menu>
            ...
            <v-list>
              ...
              <v-list-item @click="viewSkillGaps(item)">
                <v-list-item-title>View Skill Gaps</v-list-item-title>
              </v-list-item>
              <v-list-item @click="viewMatching(item)">
                <v-list-item-title>View Matching</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
        </template>
    ...
    <v-dialog v-model="showDetailsDialog" max-width="700px">
      <v-card v-if="selectedForecast">
        <v-card-title>{{ selectedForecast.role_name }} - Forecast Details</v-card-title>
        <v-divider></v-divider>
        <v-card-text class="pt-4">
          <v-row class="mb-4">
            <v-col cols="12" md="6">
              <div class="text-caption text-medium-emphasis">Current Headcount</div>
              <div class="text-h6">{{ selectedForecast.current_headcount }} FTE</div>
            </v-col>
            <v-col cols="12" md="6">
              <div class="text-caption text-medium-emphasis">Projected Headcount</div>
              <div class="text-h6">{{ selectedForecast.projected_headcount }} FTE</div>
            </v-col>
          </v-row>

          <v-row class="mb-4">
            <v-col cols="12" md="6">
              <div class="text-caption text-medium-emphasis">Growth Rate</div>
              <div class="text-h6">{{ selectedForecast.growth_rate }}%</div>
            </v-col>
            <v-col cols="12" md="6">
              <div class="text-caption text-medium-emphasis">Change in FTE</div>
              <div class="text-h6" :class="getChangeClass(selectedForecast)">
        ...
        <v-card-actions>
          ...
          <v-btn @click="saveChanges" color="primary" variant="elevated">Save Changes</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
