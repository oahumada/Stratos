...
const filteredStrategies = () => {
  return strategies.value.filter((s) => {
    ...
    const matchStrategy = strategyFilter.value ? s.strategy === strategyFilter.value : true
  ...
  <div class="closure-strategies">
    <v-row class="mb-4">
      <v-col cols="12" md="8">
        ...
        <p class="text-medium-emphasis mb-0">Revisa y filtra las estrategias sugeridas (6Bs)</p>
      </v-col>
      <v-col cols="12" md="4" class="text-right">
        <v-btn color="primary" :loading="refreshing" @click="refreshSuggested" prepend-icon="mdi-refresh">
      ...
      >
        ...
        <template #item.skill_name="{ item }">
          <div class="d-flex align-center">
            ...
            <div>
              <div class="font-weight-medium">{{ item.skill_name || 'Skill' }}</div>
              <div class="text-caption text-medium-emphasis">{{ item.strategy_name || 'Estrategia' }}</div>
            </div>
          </div>
        </template>
        ...
        <template #no-data>
          <div class="pa-6 text-center text-medium-emphasis">No hay estrategias para este escenario.</div>
        </template>
      </v-data-table>
    </v-card>
  </div>
