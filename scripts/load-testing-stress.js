...
export function stressTest() {
  group('Stress Test - API Endpoints', () => {
    ...
    let response = http.get(`${API_URL}/catalogs?page=1&limit=100&filter=active`);
    check(response, {
      'catalog endpoint responding': (r) => r.status === 200 || r.status === 401,
      'acceptable latency': (r) => r.timings.duration < 3000,
    });

    // Another heavy endpoint: Scenarios
    response = http.get(`${API_URL}/strategic-planning/scenarios?include=roles,users`);
    check(response, {
      'scenarios endpoint responding': (r) => r.status === 200 || r.status === 401,
    ...
    check(response, {
      'notification prefs responding': (r) => r.status === 200 || r.status === 401,
    });
...
export function spikeTest() {
  group('Spike Test - Sudden Traffic Increase', () => {
    ...
    check(response, {
      'spike handling': (r) => r.status === 200 || r.status === 401 || r.status === 429,
...
export function rateLimitStressTest() {
  group('Rate Limit Boundary Under Stress', () => {
    for (let i = 0; i < 20; i++) {
      ...
      check(response, {
        'rate limit header present': (r) => r.headers['X-RateLimit-Limit'] !== undefined || r.status === 401,
...
export function resourceExhaustionTest() {
  group('Resource Exhaustion Detection', () => {
    ...
    let successCount = responses.filter(s => s === 200 || s === 401).length;
