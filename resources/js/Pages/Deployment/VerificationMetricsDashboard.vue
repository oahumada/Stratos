<template>
  <div class="space-y-8 pb-12">
    ...
    <!-- Loading State -->
    <div v-if="loading" class="rounded-lg border border-gray-200 p-12 text-center dark:border-gray-700">
      <Icon icon="ph:spinner-bold" :size="32" class="mx-auto animate-spin text-gray-400" />
      <p class="mt-4 text-gray-600 dark:text-gray-400">{{ $t('common.loading') }}...</p>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-950">
    ...
    >
      <Icon icon="ph:chart-line-up-bold" :size="48" class="mx-auto text-gray-400" />
    ...
    <div v-else class="space-y-8">
      ...
      <div class="rounded-lg border border-gray-200 p-6 dark:border-gray-700">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-3">
          <div class="rounded-lg bg-green-50 p-4 dark:bg-green-950">
            ...
            <p class="mt-1 text-xs text-green-700 dark:text-green-200">
              {{
                ((metrics.recommendation_breakdown.accepted / metrics.total_verifications) * 100).toFixed(1)
              }}% {{ $t('common.of_total') }}
            </p>
          </div>
          <div class="rounded-lg bg-yellow-50 p-4 dark:bg-yellow-950">
            ...
            <p class="mt-2 text-3xl font-bold text-yellow-600 dark:text-yellow-300">
              {{ metrics.recommendation_breakdown.review_needed }}
            </p>
            <p class="mt-1 text-xs text-yellow-700 dark:text-yellow-200">
              {{
                ((metrics.recommendation_breakdown.review_needed / metrics.total_verifications) * 100).toFixed(1)
              }}% {{ $t('common.of_total') }}
            </p>
          </div>
          <div class="rounded-lg bg-red-50 p-4 dark:bg-red-950">
            ...
            <p class="mt-2 text-3xl font-bold text-red-600 dark:text-red-300">
              {{ metrics.recommendation_breakdown.rejected }}
            </p>
            <p class="mt-1 text-xs text-red-700 dark:text-red-200">
              {{
                ((metrics.recommendation_breakdown.rejected / metrics.total_verifications) * 100).toFixed(1)
              }}% {{ $t('common.of_total') }}
            </p>
          </div>
        </div>
      ...
      <!-- Metrics by Type -->
      <div v-if="metricsByType && metricsByType.length" class="rounded-lg border border-gray-200 p-6 dark:border-gray-700">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
        ...
        <div class="mt-4 overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="agent in metricsByType" :key="agent.agent_name" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ agent.agent_name }}</td>
                ...
                <td class="px-4 py-3 text-right">
                  <span
                    ...
                    :class="getConfidenceClass(agent.avg_confidence)"
                  >
...
const getConfidenceClass = (score: number) => {
  ...
  if (score >= 75) return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-950 dark:text-yellow-200'
...
const refreshMetrics = async () => {
  ...
  try {
    const [metricsRes, typeRes] = await Promise.all([
      fetch(`/api/deployment/verification-metrics?hours=${windowHours.value}`),
      fetch(`/api/deployment/verification-metrics/by-type?hours=${windowHours.value}`),
    ])
...
const exportMetrics = async () => {
  try {
    const res = await fetch(`/api/deployment/verification-metrics/export?hours=${windowHours.value}`)
