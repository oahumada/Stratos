...
<template>
    ...
    >
        ...
        >
            ...
            >
                ...
                <div class="mt-2 h-1 w-full rounded-full bg-white/20">
                    <div
                        :class="`h-1 rounded-full ${indicator.status === 'healthy' ? 'w-full bg-green-500' : indicator.status === 'warning' ? 'w-3/4 bg-yellow-500' : 'w-1/2 bg-red-500'}`"
                    ></div>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            >
                <h2
                    class="mb-4 flex items-center text-lg font-semibold text-white"
                >
                    <span
        ...
        >
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <!-- Ready Indicator -->
                <div
                    :class="`rounded-lg p-4 ${transitionReadiness.isReady ? 'bg-green-500/20' : 'bg-yellow-500/20'}`"
                >
                    <p class="text-sm font-medium text-white/70">Status</p>
                    <p
                        :class="`mt-2 text-2xl font-bold ${transitionReadiness.isReady ? 'text-green-300' : 'text-yellow-300'}`"
                    >
                        {{
                ...
                >
                    ...
                    <div
                        v-if="transitionReadiness.blockers.length === 0"
                    ...
                    <div v-else class="mt-2 space-y-1">
                        ...
                        >
                            {{ blocker.metric }}: {{ blocker.current }} (need
                            {{ blocker.required }})
                        </div>
                    </div>
                </div>
