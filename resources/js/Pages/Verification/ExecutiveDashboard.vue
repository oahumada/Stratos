...
<template>
    <div class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-8">
        ...
        <div class="mb-8 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right">
                    ...
                    <div :class="`${getSystemStatusColor(systemStatus)} rounded-full px-4 py-2 text-sm font-semibold`">
            ...
            >
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-green-400">{{ card.change }}</p>
                    </div>
                    <div :class="`text-2xl ${card.color === 'indigo' ? 'text-indigo-400' : card.color === 'success' ? 'text-green-400' : 'text-yellow-400'}`">
                        <!-- Icon would go here -->
                        📊
                    </div>
                </div>
        ...
        <div class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div class="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
                <div class="mb-4 flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-white">Transitions (7 Days)</h2>
            ...
            <!-- Health Overview -->
            <div class="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
                <h2 class="mb-6 text-lg font-semibold text-white">System Health</h2>
                <div class="space-y-4">
                    ...
                    <div v-else-if="metrics" class="space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-green-400">99.9%</span>
                        </div>
                        <div class="h-1 w-full rounded-full bg-white/10">
                            <div class="h-1 w-[99.9%] rounded-full bg-green-500"></div>
                        </div>

                        <div class="flex items-center justify-between pt-4">
                            <span class="text-white/70">Availability</span>
                            <span class="font-semibold text-green-400">99.95%</span>
                        </div>
                        <div class="h-1 w-full rounded-full bg-white/10">
                            <div class="h-1 w-[99.95%] rounded-full bg-green-500"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <!-- Recent Transitions -->
            <div class="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl lg:col-span-2">
                <h2 class="mb-4 text-lg font-semibold text-white">Recent Transitions</h2>
                <div class="space-y-2">
                    <div v-if="metrics?.recentTransitions">
                        <div
                            v-for="transition in metrics.recentTransitions.slice(0, 5)"
                            :key="transition.id"
                            class="flex items-center justify-between rounded-lg bg-white/5 p-3 text-sm"
                        >
                            <span class="text-white">{{ transition.phase }}</span>
                            <span class="text-xs text-green-400">{{ transition.date }}</span>
                        </div>
                    </div>
            ...
            <!-- Quick Stats -->
            <div class="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
                <h2 class="mb-4 text-lg font-semibold text-white">Quick Stats</h2>
                <div class="space-y-3">
                    <div class="text-sm text-white/70">
                        <span class="font-semibold text-white">Last Updated</span>
                        <p class="mt-1 text-xs text-white/50">{{ formatDate(lastUpdated) }}</p>
                    </div>
                    <div class="text-sm text-white/70">
                        <span class="font-semibold text-white">Sample Size</span>
                        <p class="mt-1 text-lg font-bold text-white">{{ metrics?.sampleSize || 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
