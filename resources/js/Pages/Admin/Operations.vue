<template>
    <div class="space-y-8">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 flex items-center gap-2 text-gray-600 dark:text-gray-400">
                    <span v-if="isStreamConnected" class="inline-block h-2 w-2 animate-pulse rounded-full bg-green-500"></span>
                    <span v-else class="inline-block h-2 w-2 rounded-full bg-gray-500"></span>
                    {{ isStreamConnected ? 'Real-time connected' : 'Offline mode' }}
                </p>
            </div>
            <button
        ...
        <div v-if="activeOperations.length > 0" class="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/20">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    ...
                    <div class="mt-2 space-y-2">
                        <div v-for="op in activeOperations" :key="op.id" class="flex items-center justify-between rounded bg-white p-2 dark:bg-gray-800">
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ op.operation_name }}
                                </p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ op.operation_type }} • Started {{ formatTime(op.started_at) }}
                                </p>
                            </div>
        ...
        <div class="rounded-lg bg-white shadow dark:bg-gray-800">
            ...
            <table v-else class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase dark:text-gray-300">
                            Operation
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase dark:text-gray-300">
                            Type
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr
                        ...
                        :class="{ 'bg-blue-50 dark:bg-blue-900/20': isOperationRunning(op.id) }"
                    >
...
function connectToStream() {
    try {
        ...
        eventSource.addEventListener('operation.queued', (event: MessageEvent) => {
            const data = JSON.parse(event.data);
            handleOperationQueued(data);
        });

        eventSource.addEventListener('operation.started', (event: MessageEvent) => {
            const data = JSON.parse(event.data);
            handleOperationStarted(data);
        });

        eventSource.addEventListener('operation.completed', (event: MessageEvent) => {
            const data = JSON.parse(event.data);
            handleOperationCompleted(data);
        });

        eventSource.addEventListener('operation.failed', (event: MessageEvent) => {
            const data = JSON.parse(event.data);
            handleOperationFailed(data);
        });

        eventSource.addEventListener('operation.rolled_back', (event: MessageEvent) => {
            const data = JSON.parse(event.data);
...
function isOperationRunning(operationId: number): boolean {
    return operations.value.some((op) => op.id === operationId && op.status === 'running');
}
