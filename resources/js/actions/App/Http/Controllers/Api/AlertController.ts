...
updateThreshold.url = (
    ...
    return (
        updateThreshold.definition.url
            .replace('{threshold}', parsedArgs.threshold.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
...
destroyThreshold.url = (
    ...
    return (
        destroyThreshold.definition.url
            .replace('{threshold}', parsedArgs.threshold.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
...
export const showHistory = (
    args:
        | { alert: string | number | { id: string | number } }
        | [alert: string | number | { id: string | number }]
        | string
        | number
        | { id: string | number },
...
showHistory.url = (
    ...
    const parsedArgs = {
        alert: typeof args.alert === 'object' ? args.alert.id : args.alert,
    };

    return (
        showHistory.definition.url
...
acknowledgeAlert.url = (
    ...
    return (
        acknowledgeAlert.definition.url
            .replace('{alert}', parsedArgs.alert.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
