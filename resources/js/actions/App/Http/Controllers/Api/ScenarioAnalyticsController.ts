...
analytics.url = (
    ...
    const parsedArgs = {
        scenario:
            typeof args.scenario === 'object'
                ? args.scenario.id
                : args.scenario,
    };

    return (
        analytics.definition.url
...
financialImpact.url = (
    ...
    return (
        financialImpact.definition.url
            .replace('{scenario}', parsedArgs.scenario.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
...
const ScenarioAnalyticsController = {
    compareScenarios,
    analytics,
    financialImpact,
    riskAssessment,
