...
describe('useCompetencySkills composable', () => {
    it('createAndAttachSkill posts skill payload and returns created skill', async () => {
        ...
        (mockApi.post as unknown as vi.Mock).mockResolvedValueOnce({
            data: created,
        });
        ...
        expect(mockApi.post).toHaveBeenCalledWith(
            '/api/competencies/1/skills',
            { skill: { name: 'Created Skill' } },
        );
    ...
    it('listSkillsForCompetency calls GET and returns list', async () => {
        ...
        (mockApi.get as unknown as vi.Mock).mockResolvedValueOnce({
            data: list,
        });
