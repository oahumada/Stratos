<template>
    <div class="overflow-hidden rounded-lg bg-white shadow dark:bg-gray-800">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <button
                                        ...
                                        class="rounded bg-green-600 px-2 py-1 text-xs text-white transition-colors hover:bg-green-700 disabled:bg-gray-400"
                                    >
                                    ...
                                    <button
                                        ...
                                        :disabled="isProcessing"
                                        class="rounded bg-blue-600 px-2 py-1 text-xs text-white transition-colors hover:bg-blue-700 disabled:bg-gray-400"
                                    >
                                        Resolver
                                    </button>
                                    <button
                                        ...
                                        :disabled="isProcessing"
                                        class="rounded bg-gray-500 px-2 py-1 text-xs text-white transition-colors hover:bg-gray-600 disabled:bg-gray-400"
                                    >
                                        Silenciar
                                    </button>
                                    <button
                                        @click="viewDetails(alert)"
                                        class="rounded bg-gray-300 px-2 py-1 text-xs text-gray-900 transition-colors hover:bg-gray-400 dark:bg-gray-600 dark:text-white dark:hover:bg-gray-700"
                                    >
                                        Ver
                                    </button>
                                </div>
        ...
        <!-- Pagination -->
        <div
            v-if="Math.ceil(totalAlerts / itemsPerPage) > 1"
        ...
        >
            <p class="text-sm text-gray-600 dark:text-gray-400">
                ...
                {{ Math.ceil(totalAlerts / itemsPerPage) }}
            </p>
            <div class="flex gap-2">
                <button
                    @click="previousPage"
                    :disabled="currentPage === 1"
                    class="rounded bg-gray-200 px-3 py-1 text-gray-900 disabled:opacity-50 dark:bg-gray-600 dark:text-white"
                >
                    Anterior
                </button>
                <button
                    @click="nextPage"
                    :disabled="
                        currentPage >= Math.ceil(totalAlerts / itemsPerPage)
                    "
                    class="rounded bg-gray-200 px-3 py-1 text-gray-900 disabled:opacity-50 dark:bg-gray-600 dark:text-white"
                >
                    Siguiente
...
const filteredAlerts = computed(() => {
    ...
    if (currentFilter.value === 'unacknowledged') {
        filtered = filtered.filter(
            (a) => !a.acknowledged_at && a.status === 'triggered',
        );
...
const nextPage = () => {
    if (currentPage.value < Math.ceil(totalAlerts.value / itemsPerPage))
        currentPage.value++;
};
</script>
