...
<template>
    <div class="space-y-4">
        ...
        >
            <!-- Timeline line -->
            <div
                class="absolute top-12 bottom-0 left-6 w-0.5 bg-gray-200 dark:bg-gray-700"
            />
            ...
            >
                <div class="flex items-start gap-3">
                    ...
                    <div class="mt-1 flex-shrink-0">
                        ...
                        <PhWarning
                            v-else-if="
                                getIcon(operation.status) === 'processing'
                            "
                            weight="fill"
                        ...
                        <PhArrowClockwise
                            v-else-if="
                                getIcon(operation.status) === 'rolled_back'
                            "
                            weight="fill"
                        ...
                        <div
                            v-else
                            class="h-6 w-6 rounded-full bg-gray-300 dark:bg-gray-600"
                        />
                    </div>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span
                                :class="[
                                    'inline-block rounded px-2 py-1 text-xs font-medium whitespace-nowrap',
                                    {
                        ...
                        >
                            ...
                            >
                                <div
                                    class="h-full bg-blue-500 transition-all dark:bg-blue-400"
                        ...
                        <!-- Time info -->
                        <div
                            class="mt-2 flex flex-wrap gap-2 text-xs text-gray-600 dark:text-gray-400"
                        >
                            <span
                                >Created:
                                {{ formatTime(operation.createdAt) }}</span
                            >
                        ...
                        <div
                            ...
                            "
                            class="mt-2 rounded bg-red-100 p-2 text-xs text-red-700 dark:bg-red-900 dark:text-red-300"
                        >
                            {{ operation.error }}
                        </div>
                        ...
                        <div
                            ...
                            class="mt-2 rounded bg-amber-100 p-2 text-xs text-amber-700 dark:bg-amber-900 dark:text-amber-300"
                        >
                            <p class="font-semibold">Rollback initiated</p>
                            <p>Reason: {{ operation.rollback.reason }}</p>
                            <p v-if="operation.rollback.completedAt">
                                Completed:
                                {{ formatTime(operation.rollback.completedAt) }}
                            </p>
                        </div>
                    </div>
                </div>
