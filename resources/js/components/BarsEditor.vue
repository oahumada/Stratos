<template>
    <div class="bars-editor">
        ...
        <p class="bars-guide" v-if="!props.readOnly">
            BARS = Behaviour, Attitude, Responsibility, Skills. Describe
            comportamientos observables o evidencias para cada sección. Ejemplo:
            "Behaviour: Comunica progreso semanalmente".
        </p>
        <div v-if="!props.readOnly" class="bars-example">
            <strong>Ejemplo rápido</strong>
            <div class="example-line">
                Behaviour: Comunica resultados claramente
            </div>
            <div class="example-line">Attitude: Proactivo</div>
            <div class="example-line">
                Responsibility: Mantener pipelines de datos
            </div>
        ...
        <div class="mode-toggle">
            <button
                type="button"
                :class="['mode-btn', { active: mode === 'structured' }]"
        ...
        <div v-if="mode === 'structured'" class="structured">
            <div class="section" v-for="key in keys" :key="key">
                ...
                <div class="items">
                    <div v-if="key !== 'skills'">
                        <div
                            v-for="(item, idx) in bars[key]"
                            :key="idx"
                            class="item-row"
                        >
                            <input
                                :data-testid="`${key}-${idx}`"
                                :placeholder="`${titleFor(key)} item ${Number(idx) + 1}`"
                                v-model="bars[key][idx]"
                                @input="onStructuredChange"
                                :disabled="props.readOnly"
                            />
                            <button
                                v-if="!props.readOnly"
                                type="button"
                                class="remove"
                                @click="removeItem(key, Number(idx))"
                            >
                                Eliminar
                            </button>
                        </div>
                    ...
                    <div v-else>
                        ...
                        <div v-if="!props.readOnly" class="skill-input-row">
                            <input
                                ...
                                v-model="skillQuery"
                                @input="onSkillQuery"
                                @keydown.enter.prevent="commitSkillFromQuery"
                            />
                            <div
                            ...
                            >
                                <div
                                    v-for="(sug, idx) in skillSuggestions"
                                    :key="idx"
                                    class="suggestion"
                ...
                <button
                    v-if="!props.readOnly && key === 'skills'"
                    type="button"
                    class="add"
                    data-testid="add-skills"
                    @click="addBlankSkill"
                >
                    Agregar Skill
                </button>
...
const bars = reactive<any>(
    props.modelValue ? normalize(props.modelValue) : defaultBars(),
);
...
function normalize(value: any) {
    ...
    return {
        behaviour: Array.isArray(value.behaviour)
            ? value.behaviour.slice()
            : defaultBars().behaviour,
        attitude: Array.isArray(value.attitude)
            ? value.attitude.slice()
            : defaultBars().attitude,
        responsibility: Array.isArray(value.responsibility)
            ? value.responsibility.slice()
            : defaultBars().responsibility,
...
function structuredValue() {
    return {
        ...
        skills: bars.skills.map((s: any) =>
            s && s.name ? { id: s.id, name: s.name } : { name: String(s) },
        ),
    };
}
...
async function onSkillQuery() {
    ...
    try {
        ...
        skillSuggestions.value = Array.isArray(data)
            ? data
            : (data?.data ?? []);
    } catch (err) {
        skillSuggestions.value = [];
...
const invalid = computed(() => {
    return keys.some((k) =>
        bars[k].some((v: string) => !v || String(v).trim() === ''),
    );
});
</script>

<style scoped>
.bars-editor textarea {
    width: 100%;
