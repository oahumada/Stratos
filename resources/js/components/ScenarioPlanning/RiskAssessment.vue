  ...
  <div class="risk-assessment space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <svg viewBox="0 0 200 200" class="w-full h-auto">
            ...
            <rect x="100" y="0" width="100" height="100" fill="#f59e0b" opacity="0.1" />
            <rect x="0" y="100" width="100" height="100" fill="#f59e0b" opacity="0.1" />
            <rect x="100" y="100" width="100" height="100" fill="#ef4444" opacity="0.1" />

            <!-- Labels -->
            <text x="50" y="50" text-anchor="middle" class="text-xs fill-emerald-600 dark:fill-emerald-400">LOW</text>
            <text x="150" y="50" text-anchor="middle" class="text-xs fill-amber-600 dark:fill-amber-400">MED</text>
            <text x="50" y="150" text-anchor="middle" class="text-xs fill-amber-600 dark:fill-amber-400">MED</text>
            <text x="150" y="150" text-anchor="middle" class="text-xs fill-red-600 dark:fill-red-400">HIGH</text>

            <!-- Current risk point -->
            <circle :cx="64 + probabilityScore * 0.72" :cy="64 + impactScore * 0.72" r="5" fill="#3b82f6" />
            <line x1="100" y1="100" :x2="64 + probabilityScore * 0.72" :y2="64 + impactScore * 0.72" stroke="#3b82f6" stroke-width="1" />

            <!-- Axis labels -->
            <text x="190" y="110" class="text-xs fill-gray-600 dark:fill-gray-400">Probability →</text>
            <text x="5" y="15" class="text-xs fill-gray-600 dark:fill-gray-400">← Impact</text>
          </svg>
        </div>
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              <span class="text-xs font-medium text-gray-600 dark:text-gray-400">Probability</span>
              <div class="text-sm font-semibold text-gray-900 dark:text-white capitalize">
                {{ risk.probability }}
              </div>
            </div>
            <div>
              <span class="text-xs font-medium text-gray-600 dark:text-gray-400">Impact</span>
              <div class="text-sm font-semibold text-gray-900 dark:text-white capitalize">
                {{ risk.impact }}
              </div>
            </div>
          ...
          <details class="cursor-pointer">
            <summary class="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
              Mitigation Strategies
            </summary>
            <div class="mt-2 pl-4 border-l-2 border-gray-300 dark:border-gray-600 space-y-2">
              <div
                v-for="(strategy, sidx) in risk.mitigation_strategies"
                :key="sidx"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <ul class="space-y-2">
          <li v-for="(action, idx) in recommendedActions" :key="idx" class="flex gap-3">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <ul class="space-y-2">
          <li v-for="(factor, idx) in successFactors" :key="idx" class="flex gap-3">
...
const riskData = ref<RiskItem[]>([
  {
    ...
    description: 'Market constraints for AI/ML specialists may delay hiring',
    probability: 'high',
    impact: 'high',
    score: 75,
  ...
  },
  {
    ...
    description: 'Economic downturn or industry changes could affect execution',
    probability: 'medium',
    impact: 'medium',
    score: 50,
    mitigation_strategies: [
