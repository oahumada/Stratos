...
describe('Consistency Alert Logic', () => {
    ...
    describe('Operational Role (O)', () => {
        ...
        it('should show success for level > 3 with referent flag', () => {
            ...
            expect(consistencyAlert.value.title).toBe(
                'Rol de Referencia Validado',
            );
...
describe('Show Referent Option Logic', () => {
    const shouldShowReferentOption = (
        archetype: string,
        level: number,
    ): boolean => {
        return (
            (archetype === 'O' && level > 3) || (archetype === 'T' && level > 4)
        );
    };
...
describe('Level Decrease Rationale Logic', () => {
    ...
    it('should not show when there is no previous mapping', () => {
        expect(shouldShowLevelDecreaseRationale(false, 2, undefined)).toBe(
            false,
        );
        expect(shouldShowLevelDecreaseRationale(true, 2, undefined)).toBe(
            false,
        );
    });
});
