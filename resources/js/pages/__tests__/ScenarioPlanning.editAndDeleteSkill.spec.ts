...
vi.mock('@/composables/useApi', () => ({
    useApi: () => ({
        ...
        get: getMock,
        delete: deleteMock,
        api: { patch: patchMock, get: getMock, delete: deleteMock },
    }),
}));
...
describe('ScenarioPlanning skill edit & delete modal', () => {
    ...
    beforeEach(() => {
        ...
        getMock.mockResolvedValue({
            data: { id: 555, name: 'Edited Skill', category: 'testing' },
        });
    ...
    it('edits skill fields, patches skill and pivot, closes modal and refreshes selected skill', async () => {
        ...
        const csCall = patchMock.mock.calls.find(
            (c: any) =>
                typeof c[0] === 'string' &&
                c[0].includes('/api/competency-skills/'),
        );
        expect(csCall).toBeTruthy();

        // Expect either competency_skills patch or scenario-scoped pivot patch to have been attempted
        const pivotCall = patchMock.mock.calls.find(
            (c: any) =>
                typeof c[0] === 'string' &&
                c[0].includes('/api/strategic-planning/scenarios/'),
        );
    ...
    it('removes a skill from competency (delete pivot) and updates local arrays', async () => {
        ...
        expect(
            wrapper.vm.grandChildNodes.find((g: any) => g.id === 888),
        ).toBeUndefined();
    });
