...
vi.mock('@/composables/useApi', () => ({
    useApi: () => ({
        delete: deleteMock,
        get: async () => ({ data: [] }),
        api: { delete: deleteMock, get: async () => ({ data: [] }) },
    }),
}));
...
describe('ScenarioPlanning delete node', () => {
    ...
    it('deletes competency when selectedChild present', async () => {
        ...
        if (typeof wrapper.vm.deleteFocusedNode === 'function') {
            ...
            const call = deleteMock.mock.calls.find((c: any) =>
                String(c[0]).includes(
                    '/strategic-planning/scenarios/99/capabilities/77/competencies/321',
                ),
            );
