    ...
    capabilities: [
        {
            ...
            y: 120,
            priority: 1,
            competencies: Array.from({ length: 6 }).map((_, i) => ({
                id: 100 + i,
                name: `Comp ${i + 1}`,
                skills: [],
                pivot: {},
...
describe('ScenarioPlanning integration - competency expand and centering', () => {
    ...
    beforeEach(async () => {
        ...
        apiPostMock = vi.fn(async (url: string, data?: any) => {
            ...
            if (m) {
                const id = 5555;
                const skill =
                    data && data.skill ? { id, ...data.skill } : { id };
                return { data: skill };
            }
            return { data: {} };
        });
        ...
        const utils = render(Index as any, {
            props: { scenario: makeScenario() },
            global: {
                stubs: {
    ...
    it('opens competency children and centers selected child when clicked', async () => {
        ...
        const capBtn = Array.from(
            container.querySelectorAll('.node-label'),
        ).find((el) => el.textContent?.includes('Capability A')) as
            | HTMLElement
            | undefined;
    ...
    it('creates and attaches a skill via modal and closes dialog', async () => {
        ...
        const { useCompetencySkills } =
            await import('@/composables/useCompetencySkills');
        const svc = useCompetencySkills();
        await svc.createAndAttachSkill(compId, {
            name: 'Integration Skill X',
        } as any);

        // assert API was called with competency attach endpoint and payload
        expect(apiPostMock).toHaveBeenCalled();
        const callArgs = apiPostMock.mock.calls.find(
            (c: any) =>
                c[1] && c[1].skill && c[1].skill.name === 'Integration Skill X',
        );
