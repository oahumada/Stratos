...
vi.mock('@/composables/useApi', () => ({
    useApi: () => ({
        post: postMock,
        get: async () => ({ data: [] }),
        api: { post: postMock, get: async () => ({ data: [] }) },
    }),
}));
...
describe('ScenarioPlanning attach existing competency', () => {
    ...
    it('posts competency_id to scenario-capability endpoint when attaching existing', async () => {
        ...
        if (typeof wrapper.vm.attachExistingComp === 'function') {
            ...
            const call = postMock.mock.calls.find((c: any) =>
                String(c[0]).includes(
                    '/strategic-planning/scenarios/11/capabilities/77/competencies',
                ),
            );
