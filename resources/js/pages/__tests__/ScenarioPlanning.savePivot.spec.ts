...
describe('ScenarioPlanning pivot save', () => {
    beforeEach(async () => {
        ...
        ScenarioPlanning = (await import('@/pages/ScenarioPlanning/Index.vue'))
            .default;
    });
    ...
    it('sends pivot PATCH and merges updates into local node', async () => {
        ...
        wrapper.vm.focusedNode = {
            id: 10,
            raw: { scenario_capabilities: [{ strategic_weight: 5 }] },
        };

        // wait a tick in case watchers reset edit fields, then set edit pivot fields
        await wrapper.vm.$nextTick();
        wrapper.vm.editPivotStrategicWeight = 8;
        if (
            typeof wrapper.vm.editPivotStrategicWeight === 'object' &&
            'value' in wrapper.vm.editPivotStrategicWeight
        )
            wrapper.vm.editPivotStrategicWeight.value = 8;
        wrapper.vm.editPivotPriority = 2;
        if (
            typeof wrapper.vm.editPivotPriority === 'object' &&
            'value' in wrapper.vm.editPivotPriority
        )
            wrapper.vm.editPivotPriority.value = 2;
        wrapper.vm.editPivotRationale = 'test rationale';
        if (
            typeof wrapper.vm.editPivotRationale === 'object' &&
            'value' in wrapper.vm.editPivotRationale
        )
            wrapper.vm.editPivotRationale.value = 'test rationale';
        wrapper.vm.editPivotRequiredLevel = 4;
        if (
            typeof wrapper.vm.editPivotRequiredLevel === 'object' &&
            'value' in wrapper.vm.editPivotRequiredLevel
        )
            wrapper.vm.editPivotRequiredLevel.value = 4;
        wrapper.vm.editPivotIsCritical = true;
        if (
            typeof wrapper.vm.editPivotIsCritical === 'object' &&
            'value' in wrapper.vm.editPivotIsCritical
        )
            wrapper.vm.editPivotIsCritical.value = true;
        ...
        if (typeof wrapper.vm.saveFocusedNode === 'function') {
            ...
            const capPatchCall = patchMock.mock.calls.find((c: any) =>
                String(c[0]).includes('/api/capabilities/10'),
            );
            ...
            const pivotCall = patchMock.mock.calls.find((c: any) =>
                String(c[0]).includes(
                    '/strategic-planning/scenarios/42/capabilities/10',
                ),
            );
            ...
            const pivots =
                wrapper.vm.focusedNode.raw.scenario_capabilities ||
                wrapper.vm.focusedNode.raw.pivot ||
                wrapper.vm.focusedNode.raw._pivot;
