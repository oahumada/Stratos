...
const roleOptions = computed(() =>
    roles.value.map((r) => ({ ...r, label: r.name, value: r.id })),
);
...
const generateLearningPath = async () => {
    ...
    if (!result.value) {
        notify({
            type: 'warning',
            text: 'Realiza primero un análisis de brechas',
        });
        return;
    }
    ...
    try {
        console.log(
            'Generating learning path for people:',
    ...
    <div class="pa-4">
        ...
        >
            <div>
                ...
                >
                    Identifica las brechas de competencias entre el perfil
                    actual de una persona y los requisitos de un rol específico.
                </p>
            </div>
        </div>
        ...
        <div v-if="result">
            <v-card class="mb-4">
                <v-card-text>
                    <div
                        class="d-flex justify-space-between align-center mb-4 flex-wrap gap-4"
                    >
                        <div>
                        ...
                        <div class="text-right">
                            ...
                            <div
                                :class="`text-h6 text-${getMatchColor(result.analysis.match_percentage)}`"
                            >
                    ...
                    <div class="d-flex mt-3 flex-wrap gap-2">
                        <v-chip size="small" color="primary" variant="flat">
                            {{ result.analysis.summary?.skills_ok ?? 0 }}/{{
                                result.analysis.summary?.total_skills ??
                        ...
                        <v-chip
                            ...
                            :color="
                                getStatusColor(
                                    result.analysis.summary?.category || 'info',
                                )
                            "
                            variant="flat"
                            text-color="white"
                        >
                            {{
                                result.analysis.summary?.category ||
                                'sin categoría'
                            }}
                        </v-chip>
                        <v-chip
                            ...
                            color="info"
                            variant="outlined"
                            v-if="result.analysis.gaps.length === 0"
                        >
                            Sin brechas
                        </v-chip>
                    </div>
            ...
            <v-row>
                <v-col cols="12" md="7">
                    <v-card class="mb-4">
                        <v-card-title
                            >Detalle de brechas por skill</v-card-title
                        >
                        <v-card-text>
                            <v-table
                                v-if="result.analysis.gaps.length > 0"
                            ...
                            >
                                ...
                                <tbody>
                                    <tr
                                        v-for="gap in result.analysis.gaps"
                                        :key="`${gap.skill_id}`"
                                    >
                                        ...
                                        <td class="text-center">
                                            <v-chip
                                                size="small"
                                                variant="outlined"
                                                >{{ gap.current_level }}</v-chip
                                            >
                                        </td>
                                        <td class="text-center">
                                            <span
                                                :class="`text-${getStatusColor(gap.status)}`"
                                            >
                                                {{ gap.gap }}
                                            </span>
                                        </td>
