    ...
    <div class="transform-modal">
        <form @submit.prevent="submit">
            <div class="transform-header">
                ...
                <p v-if="versions.length > 0" class="text-sm">
                    Versiones existentes: {{ versions.length }}
                </p>
            </div>

            <div
                class="guide-toggle-row"
                style="
                    display: flex;
            ...
            >
                <div>
                    ...
                    <small
                        ...
                        >(¿Necesitas más detalle?)</small
                    >
                </div>
                <div style="display: flex; gap: 0.5rem; align-items: center">
                    <a
                        href="/docs/COMIENZA_AQUI_WORKFORCE_PLANNING.md"
                        target="_blank"
                        rel="noopener"
                        class="text-sm"
                        style="color: #1f79ff"
                    ...
                    >
                        <v-icon class="text-info-darken-1 align-middle text-4xl"
                            >mdi-information-variant-circle</v-icon
                        >
                    </v-btn>
                    <InfoLegend
                        v-model="showLegend"
                        title="Guía: Transformación / BARS"
                        :items="legendItems"
            ...
            <div style="margin-top: 0.5rem">
                <label style="display: flex; align-items: center; gap: 0.5rem">
                    <input
                        type="checkbox"
                        v-model="form.create_skills_incubated"
                    />
                    <span>Crear skills en incubación</span>
                </label>
                <p
                    class="text-sm text-gray-500"
                    style="margin: 0.25rem 0 0 1.8rem"
                >
                    Si está activado, las skills nuevas se crearán con estado
                    'incubation' cuando se cree la versión.
                </p>
            </div>
...
async function submit() {
    try {
        ...
        const skill_ids = skillObjs
            .filter((s: any) => s && s.id)
            .map((s: any) => s.id);
        const new_skills = skillObjs
            .filter((s: any) => s && !s.id)
            .map((s: any) => s.name);
        const payload: any = {
            name: form.name,
        ...
        const data = await store.transformCompetency(
            props.competencyId,
            payload,
        );
