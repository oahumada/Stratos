...
const loadScenarios = async () => {
    ...
    try {
        ...
        if (filters.value.fiscalYear)
            params.fiscal_year = filters.value.fiscalYear;

        const res = await api.get('/api/strategic-planning/scenarios', {
            params,
        });
        // useApi.get devuelve response.data directamente
        // El backend responde { success, data: [...], pagination }
        scenarios.value = Array.isArray((res as any).data)
            ? (res as any).data
            : Array.isArray(res)
              ? res
              : [];
...
const saveScenario = async () => {
    try {
        if (editingScenario.value) {
            await api.put(
                `/api/strategic-planning/scenarios/${editingScenario.value.id}`,
                formData.value,
            );
            showSuccess('Scenario updated successfully');
        } else {
            await api.post('/api/strategic-planning/scenarios', formData.value);
...
const deleteScenario = async (id: number) => {
    if (confirm('Are you sure you want to delete this scenario?')) {
        try {
            await api.delete(`/api/strategic-planning/scenarios/${id}`);
            ...
            <v-dialog v-model="showCreateDialog" max-width="600px">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" @click="saveScenario"
                            >Save</v-btn
                        >
                    </v-card-actions>
                </v-card>
