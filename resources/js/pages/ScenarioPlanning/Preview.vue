        ...
        <v-container>
            ...
            >
                <div class="d-flex align-center">
                    <v-icon v-if="snackbarType === 'error'" class="mr-2"
                        >mdi-alert-circle</v-icon
                    >
                    <v-icon v-else-if="snackbarType === 'success'" class="mr-2"
                        >mdi-check-circle</v-icon
                    >
                    <div class="flex-grow-1">{{ snackbarMessage }}</div>
                    <v-btn
                        v-if="snackbarActionLabel"
                        text
                        @click="performSnackbarAction"
                        >{{ snackbarActionLabel }}</v-btn
                    >
                </div>
            </v-snackbar>
                ...
                <v-col cols="12" md="8">
                    <v-card>
                        <v-card-text>
                            ...
                            <div class="text-h4">
                                {{
                                    (scenario.confidence_score * 100).toFixed(
                                        0,
                                    )
                                }}%
                            </div>
...
async function loadCompacted() {
    try {
        ...
        if (res.ok) {
            ...
            if (json?.suggested_roles)
                displayedRoles.value = json.suggested_roles;
        } else {
        ...
        errorMessage.value =
            'No se pudieron cargar las roles sugeridos. Intenta nuevamente.';
        showSnackbar(
            'Error cargando roles sugeridos',
            'Reintentar',
            'retryLoad',
        );
    }
        ...
        payload,
        {
            onSuccess: () => {
                successMessage.value =
                    'Escenario aceptado. Redirigiendo a Paso 2...';
            ...
            onError: (errs: any) => {
                ...
                errorMessage.value =
                    'No se pudo aceptar el escenario. Verifica e intenta de nuevo.';
                showSnackbar(
                    'No se pudo aceptar el escenario',
                    'Reintentar',
                    'retryAccept',
                );
            },
