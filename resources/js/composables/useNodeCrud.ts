...
export function useNodeCrud(config: CrudConfig) {
    ...
    async function ensureCsrf() {
        try {
            ...
            console.debug(
                '[ensureCsrf] CSRF cookie request failed, proceeding anyway',
                err,
            );
        }
    }
    ...
    async function createAndAttach(
        ...
        try {
            const endpoint =
                customEndpoint ||
                `${config.parentRoute}/${parentId}/${config.entityNamePlural}`;
            console.debug(
                `[useNodeCrud.createAndAttach] POST ${endpoint}`,
                payload,
            );
            ...
            showSuccess(
                `${capitalizeFirst(config.entityName)} creada exitosamente`,
            );
            return created;
        } catch (error: any) {
            console.error(
                `[useNodeCrud.createAndAttach] Error creating ${config.entityName}:`,
                error?.response?.data ?? error,
            );
            showError(
                `Error creando ${config.entityName}: ${error?.response?.data?.message || error?.message || 'Unknown error'}`,
            );
            throw error;
        } finally {
    ...
    async function updateEntity(
        ...
        try {
            const endpoint =
                customEndpoint || `/api/${config.entityNamePlural}/${entityId}`;
            console.debug(
                `[useNodeCrud.updateEntity] PATCH ${endpoint}`,
                payload,
            );
            ...
            console.error(
                `[useNodeCrud.updateEntity] Error updating ${config.entityName}:`,
                error?.response?.data ?? error,
            );
            showError(
                `Error actualizando ${config.entityName}: ${error?.response?.data?.message || error?.message || 'Unknown error'}`,
            );
            throw error;
        } finally {
            saving.value = false;
    ...
    async function updatePivot(
        ...
        try {
            ...
            console.debug(
                `[useNodeCrud.updatePivot] PATCH ${endpoint}`,
                pivotPayload,
            );
            ...
            console.error(
                `[useNodeCrud.updatePivot] Error updating pivot:`,
                error?.response?.data ?? error,
            );
    ...
    async function deleteEntity(
        ...
        try {
            ...
            console.error(
                `[useNodeCrud.deleteEntity] Error deleting ${config.entityName}:`,
                error?.response?.data ?? error,
            );
            showError(
                `Error eliminando ${config.entityName}: ${error?.response?.data?.message || error?.message || 'Unknown error'}`,
            );
            throw error;
        } finally {
    ...
    async function fetchEntity(
        ...
        try {
            ...
            console.error(
                `[useNodeCrud.fetchEntity] Error fetching ${config.entityName}:`,
                error?.response?.data ?? error,
            );
