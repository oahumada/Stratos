...
describe('Composables Integration - Complete Workflow', () => {
    ...
    describe('Step 5: Edge rendering', () => {
        beforeEach(() => {
            ...
            state.grandChildNodes.value = state.grandChildNodes.value.map(
                (n) => ({
                    ...n,
                    x: n.x ?? 0,
                    y: n.y ?? 0,
    ...
    describe('Full workflow integration', () => {
        ...
        it('maintains state consistency across composable interactions', () => {
            ...
            // Verify consistency
            expect(state.childEdges.value[0].source).toBe(
                state.focusedNode.value.id,
            );
            expect(
                state.childNodes.value.some(
                    (cn) => cn.id === state.childEdges.value[0].target,
                ),
            ).toBe(true);
        });
