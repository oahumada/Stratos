...
describe('computeMatrixPositions', () => {
    it('returns at most 10 positions for total > 10 and arranges in 2x5 grid', () => {
        ...
        const cx = 500;
        const topY = 200;
        const positions = computeMatrixPositions(total, cx, topY, {
            rows: 2,
            cols: 5,
            hSpacing: 120,
            vSpacing: 100,
        ...
        const rowYs = Array.from(new Set(positions.map((p) => p.y))).sort(
            (a, b) => a - b,
        );
    ...
    it('for total=10 returns exactly 2 rows x 5 cols and fills left-to-right by row', () => {
        ...
        for (let i = 1; i < firstRowXs.length; i++)
            expect(firstRowXs[i]).toBeGreaterThan(firstRowXs[i - 1]);
        for (let i = 1; i < secondRowXs.length; i++)
            expect(secondRowXs[i]).toBeGreaterThan(secondRowXs[i - 1]);
    });
});
