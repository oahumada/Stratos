...
export function useVerificationDashboardRealtime() {
    ...
    const setupSSEConnection = () => {
        try {
            eventSource = new EventSource(
                '/api/deployment/verification/realtime-events?stream=true',
            );
            ...
            eventSource.onerror = () => {
                console.warn(
                    '⚠ SSE error, disconnecting and falling back to polling',
                );
                eventSource?.close();
                eventSource = null;
                connectionStatus.value.connected = false;
    ...
    const getUserOrgId = (): number => {
        ...
        return (
            (window as any).__INITIAL_STATE__?.props?.auth?.user
                ?.organization_id || 1
        );
    };
