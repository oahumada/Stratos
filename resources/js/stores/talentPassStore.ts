...
import type {
  ...
  TalentPassState,
  TalentPassSkill,
  TalentPassCredential,
  TalentPassExperience,
  CreateTalentPassRequest,
  UpdateTalentPassRequest,
  SearchFilters,
  PaginationParams,
  TalentPassComputedState,
...
export const useTalentPassStore = defineStore('talents-pass', () => {
  ...
  const currentCompleteness = computed(() => {
    return currentTalentPass.value ? calculateCompleteness(currentTalentPass.value) : 0;
  });
  ...
  const isPublished = computed(() => {
    return currentTalentPass.value?.status === 'active' || currentTalentPass.value?.status === 'approved';
  });
  ...
  const filteredTalentPasses = computed(() => {
    return talentPasses.value.filter((tp) => {
      ...
      if (filters.value.visibility && tp.visibility !== filters.value.visibility) {
        return false;
      }
  ...
  const fetchTalentPasses = async (params?: PaginationParams) => {
    ...
    try {
      ...
      const response = await apiClient.get(`/api/talent-passes?${queryParams.toString()}`);
      talentPasses.value = response.data.data;
      totalPages.value = response.data.meta.last_page;
    } catch (err: any) {
      error.value = err.response?.data?.message || 'Error fetching Talent Passes';
    } finally {
      loading.value = false;
    }
  };
  ...
  const fetchTalentPass = async (id: number) => {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Error fetching Talent Pass';
    } finally {
      loading.value = false;
    }
  };
  ...
  const fetchPublicTalentPass = async (ulid: string) => {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Error fetching public Talent Pass';
    } finally {
      loading.value = false;
    }
  };
  ...
  const createTalentPass = async (payload: CreateTalentPassRequest) => {
    ...
    try {
      const response = await apiClient.post('/api/talent-passes', payload);
      talentPasses.value.unshift(response.data.data);
      currentTalentPass.value = response.data.data;
      isCreateModalOpen.value = false;
    } catch (err: any) {
      error.value = err.response?.data?.message || 'Error creating Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
   ...
   */
  const updateTalentPass = async (id: number, payload: UpdateTalentPassRequest) => {
    loading.value = true;
    error.value = null;

    try {
      const response = await apiClient.put(`/api/talent-passes/${id}`, payload);
      ...
      error.value = err.response?.data?.message || 'Error updating Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const deleteTalentPass = async (id: number) => {
    ...
    try {
      ...
      talentPasses.value = talentPasses.value.filter((tp) => tp.id !== id);
      if (currentTalentPass.value?.id === id) {
        currentTalentPass.value = null;
      }
    } catch (err: any) {
      error.value = err.response?.data?.message || 'Error deleting Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const publishTalentPass = async (id: number, reason?: string) => {
    ...
    try {
      const response = await apiClient.post(`/api/talent-passes/${id}/publish`, { reason });
      currentTalentPass.value = response.data.data;
      const index = talentPasses.value.findIndex((tp) => tp.id === id);
      if (index !== -1) {
        talentPasses.value[index] = response.data.data;
      ...
      error.value = err.response?.data?.message || 'Error publishing Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const archiveTalentPass = async (id: number, reason?: string) => {
    ...
    try {
      const response = await apiClient.post(`/api/talent-passes/${id}/archive`, { reason });
      currentTalentPass.value = response.data.data;
      const index = talentPasses.value.findIndex((tp) => tp.id === id);
      if (index !== -1) {
        talentPasses.value[index] = response.data.data;
      ...
      error.value = err.response?.data?.message || 'Error archiving Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const cloneTalentPass = async (id: number, title: string) => {
    ...
    try {
      const response = await apiClient.post(`/api/talent-passes/${id}/clone`, { title });
      talentPasses.value.unshift(response.data.data);
      currentTalentPass.value = response.data.data;
    } catch (err: any) {
      error.value = err.response?.data?.message || 'Error cloning Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
   ...
   * Export Talent Pass
   */
  const exportTalentPass = async (id: number, format: 'pdf' | 'json' | 'linkedin') => {
    loading.value = true;
    error.value = null;

    try {
      const response = await apiClient.post(`/api/talent-passes/${id}/export`, { format });
      return response.data.data; // Returns { url, format, expires_at }
    } catch (err: any) {
      error.value = err.response?.data?.message || 'Error exporting Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const shareTalentPass = async (id: number, message?: string) => {
    ...
    try {
      const response = await apiClient.post(`/api/talent-passes/${id}/share`, { message });
      return response.data.data; // Returns { public_url, public_id, expires_at }
    } catch (err: any) {
      error.value = err.response?.data?.message || 'Error sharing Talent Pass';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const addSkill = async (talentPassId: number, skill: any) => {
    try {
      const response = await apiClient.post(`/api/talent-passes/${talentPassId}/skills`, skill);
  ...
  const removeSkill = async (talentPassId: number, skillId: number) => {
    try {
      await apiClient.delete(`/api/talent-passes/${talentPassId}/skills/${skillId}`);
      if (currentTalentPass.value?.skills) {
        currentTalentPass.value.skills = currentTalentPass.value.skills.filter(
          (s) => s.id !== skillId
        );
      }
    } catch (err: any) {
