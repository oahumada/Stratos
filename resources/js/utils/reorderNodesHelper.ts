 ...
 */
export function reorderNodesHelper(
    nodes: NodeItem[],
    width: number,
    height: number,
    scenarioNode?: NodeItem,
) {
    ...
    if (
        scenarioId != null &&
        workingNodes.some((n) => n && n.id === scenarioId)
    ) {
    ...
    if (total === 1) {
        ...
        const spacingY = Math.min(
            140,
            Math.floor((height - 160) / Math.max(1, rows)),
        );
