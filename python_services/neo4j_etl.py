...
def fetch_updated_persons(conn, since):
    q = """
    SELECT id, name, email, organization_id, updated_at
    FROM users
    WHERE updated_at > %s
...
def fetch_updated_competencies(conn, since):
    q = """
    SELECT id, key, name, description, updated_at
    FROM competencies
    WHERE updated_at > %s
    ORDER BY updated_at ASC
    LIMIT 1000
...
def fetch_person_skills(conn, since):
    q = """
    SELECT ps.person_id, ps.skill_id, ps.level, ps.score, ps.updated_at, s.name as skill_name
...
def upsert_competency(tx, comp):
    tx.run(
        ...
        RETURN c
        """,
        id=comp['id'], key=comp.get('key'), name=comp.get('name'), description=comp.get('description'), updated_at=comp.get('updated_at')
    )
...
def upsert_person_skill(tx, ps):
    tx.run(
        ...
        SET r.level = $level, r.score = $score, r.updated_at = $updated_at
        RETURN p, s, r
        """,
        person_id=ps['person_id'], skill_id=ps['skill_id'], skill_name=ps.get('skill_name'), level=ps.get('level'), score=ps.get('score'), updated_at=ps.get('updated_at')
    )
    ...
    try:
        ...
        with driver.session() as session:
            for comp in fetch_updated_competencies(pg, comp_since):
                session.write_transaction(upsert_competency, comp)
        ...
        # Persons
        person_since = get_last_sync(pg, 'users') or '1970-01-01T00:00:00Z'
        print(f"Sync users since={person_since}")
        max_person_ts = None
        with driver.session() as session:
            for person in fetch_updated_persons(pg, person_since):
                session.write_transaction(upsert_person, person)
                if person.get('updated_at'):
                    max_person_ts = person['updated_at'] if not max_person_ts else max(max_person_ts, person['updated_at'])
        if max_person_ts:
            set_last_sync(pg, 'users', max_person_ts)

        # Person skills (pivots)
        ps_since = get_last_sync(pg, 'person_skill') or '1970-01-01T00:00:00Z'
        print(f"Sync person_skill since={ps_since}")
        max_ps_ts = None
        with driver.session() as session:
            for ps in fetch_person_skills(pg, ps_since):
                session.write_transaction(upsert_person_skill, ps)
                if ps.get('updated_at'):
                    max_ps_ts = ps['updated_at'] if not max_ps_ts else max(max_ps_ts, ps['updated_at'])
        if max_ps_ts:
            set_last_sync(pg, 'person_skill', max_ps_ts)
