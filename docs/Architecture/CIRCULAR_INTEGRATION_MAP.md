...
```mermaid
graph TD
    A[Misión/Visión/Valores] -->|Inyecta ADN| B(Guardian de Cultura)
    B -->|Filtra| C[Selección Inteligente]
    B -->|Evalúa| D[Talento 360 / Cerbero]

    E[Scenario IQ] -->|Crea Vacante Virtual| C
    C -->|Baseline de Potencial| D
    D -->|Detecta Gaps| F[Learning Paths]
